...
UINT16                  Timer_S;            // Tracks seconds elapsed for log timestamp
UINT16                  Timer_tms;          // Tracks tenths of milliseconds elapsed for log timestamp
...
UINT16           tPDDebounce;     // Timer used for first level debouncing
UINT16           tCCDebounce;     // Timer used for second level debouncing
...
UINT16           tOverPDDebounce;    //Timer used to ignore traffic less than tPDDebounce
...
void TypeCTickAt100us(void)
{
    if ((StateTimer<T_TIMER_DISABLE) && (StateTimer>0))
        StateTimer--;
    if ((tPDDebounce<T_TIMER_DISABLE) && (tPDDebounce>0))
        tPDDebounce--;
    if ((tCCDebounce<T_TIMER_DISABLE) && (tCCDebounce>0))
        tCCDebounce--;
    if ((ToggleTimer<T_TIMER_DISABLE) && (ToggleTimer>0))
        ToggleTimer--;
    if (PRSwapTimer)
        PRSwapTimer--;
    if ((tOverPDDebounce<T_TIMER_DISABLE) && (tOverPDDebounce>0))
        tOverPDDebounce--;
...
void InitializeTypeCVariables(void)
{
    ...
    tPDDebounce = T_TIMER_DISABLE;         // Disable the 1st debounce timer
    tCCDebounce = T_TIMER_DISABLE;         // Disable the 2nd debounce timer
...
void SetStateDisabled(void)
{
    ...
    tPDDebounce = T_TIMER_DISABLE;                                     // Disable the 1st level debounce timer
    tCCDebounce = T_TIMER_DISABLE;                                     // Disable the 2nd level debounce timer
    ToggleTimer = T_TIMER_DISABLE;                                        // Disable the toggle timer
...
void SetStateUnattached(void)
{
    ...
    tCCDebounce = T_TIMER_DISABLE;                                     // Disable the 2nd level debounce timer, not used in this state
...
void SetStateAttachWaitSource(void)
{
    ...
    tOverPDDebounce = T_TIMER_DISABLE;  // Disable PD filter timer
    StateTimer = T_TIMER_DISABLE;                                         // Disable the state timer, not used in this state
    tPDDebounce = tPDDebounceMin;                                // Only debounce the lines for tPDDebounce so that we can debounce a detach condition
    tCCDebounce = tCCDebounceMin;                                     // Disable the 2nd level debouncing initially to force completion of a 1st level debouncing
...
void SetStateAttachedSource(void)
{
    ...
    StateTimer = T_TIMER_DISABLE;                                         // Disable the state timer, not used in this state
    tPDDebounce = tPDDebounceMin;                                // Set the debounce timer to tPDDebounceMin for detecting a detach
    tCCDebounce = tCCDebounceMin;                                     // Disable the 2nd level debouncing, not needed in this state
    ToggleTimer = T_TIMER_DISABLE;                                        // No toggle on sources
    tOverPDDebounce = T_TIMER_DISABLE;  // Disable PD filter timer
    WriteStateLog(&TypeCStateLog, ConnState, Timer_tms, Timer_S);
}
...
void SetStateTryWaitSink(void)
{
    ...
    tPDDebounce = tPDDebounceMin;                                // The 1st level debouncing is based upon tPDDebounce
    tCCDebounce = T_TIMER_DISABLE;                                     // Disable the 2nd level debouncing initially until we validate the 1st level
...
void SetStateTrySource(void)
{
    ...
    tPDDebounce = tPDDebounceMin;                                // Debouncing is based solely off of tPDDebounce
...
void SetStateTryWaitSource(void)
{
    ...
    tPDDebounce = tPDDebounceMin;                                // Only debounce the lines for tPDDebounce so that we can debounce a detach condition
    tCCDebounce = T_TIMER_DISABLE;                                     // Not needed in this state
    ToggleTimer = T_TIMER_DISABLE;                                             // No toggle on sources
    WriteStateLog(&TypeCStateLog, ConnState, Timer_tms, Timer_S);
}
...
void SetStateDebugAccessory(void)
{
    ...
    SinkCurrent = utccNone;                                         // Not used in accessories
    StateTimer = T_TIMER_DISABLE;                                         // Disable the state timer, not used in this state
    tPDDebounce = T_TIMER_DISABLE;                                // Once in this state, we are waiting for the lines to be stable for tCCDebounce before changing states
...
void SetStatePoweredAccessory(void)
{
    ...
    tPDDebounce = tPDDebounceMin;                                // Set the debounce timer to the minimum tPDDebounce to check for detaches
    tCCDebounce = T_TIMER_DISABLE;                                     // Disable the 2nd level debounce timer, not used in this state
...
CCTermType DecodeCCTermination(void)
{
    ...
    switch (sourceOrSink)
    {
        case Source:
            ...
            {
                AlternateDRPSourceSinkSwap();
                if(ConnState == UnattachedSource)
                {
                    return AlternateDecodeCCTerminationSource();
                }
                else
                {
...
UINT8 GetCCTermination(void)    // Figured out what this is for and correct ~Rick
{
    UINT8 status = 0;

            status |= (CC1TermCCDebounce & 0x07);          // Set the current CC1 termination
...
void debounceCC(void)
{
    ...
    if (Registers.Switches.MEAS_CC1)                                            // If we are looking at CC1
    {
        if (CC1TermPrevious != CCTermCurrent)                                // Check to see if the value has changed...
        {
            ...
            tPDDebounce = tPDDebounceMin;                                       // Restart the debounce timer with tPDDebounce (wait 10ms before detach)
    ...
    if (tPDDebounce == 0)                                                       // Check to see if our debounce timer has expired...
    {
        ...
        tPDDebounce = T_TIMER_DISABLE;
        tOverPDDebounce = T_TIMER_DISABLE;
    }
    if (tOverPDDebounce == 0)
    {
    ...
    if ((CC1TermPDDebouncePrevious != CC1TermPDDebounce) || (CC2TermPDDebouncePrevious != CC2TermPDDebounce))   //If the PDDebounce values have changed
    {
        ...
        tCCDebounce = tCCDebounceMin - tPDDebounceMin;                          // reset the tCCDebounce timers
